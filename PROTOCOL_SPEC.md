...
The Gate 1 architecture and current venue rejection record are frozen in [`BACKING_VENUE_DECISION.md`](./BACKING_VENUE_DECISION.md). `AAPL2L` requires prepaid, non-recourse, non-recallable risk capital to fund the second unit of spot exposure. `AAPL2S` requires a prepaid bounded-loss derivative with maximum loss locked onchain; naked borrow and ordinary margin/perpetual accounts are inadmissible. Both require an independently operated emergency exit route. The deterministic pre-admission model in [`lib/backing-engine.ts`](./lib/backing-engine.ts) cannot authorize a venue by itself; immutable external and onchain evidence must still satisfy the release manifest.

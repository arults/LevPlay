...
| Candidate product catalog | 136 auditable candidate definitions: 15 Ondo stocks and 5 commodity-linked ETFs at 2×/3×/5× L/S, plus 8 PreStocks references at 2× L/S; two Tessera references add four paper concepts only; catalog presence never makes a market executable |
...
| CONFIG-02 | High | Deployment JSON was not bound to one explicit content hash. | A strict schema-v2 manifest now rejects missing/extra fields, contains exactly `AAPL2L` and `AAPL2S`, distinguishes USDC clearing, source-token and reserve vaults, binds venue/product admission JSON and release artifacts by SHA-256, and must match `LEVPLAY_SVM_DEPLOYMENT_MANIFEST_HASH`. Two RPCs also decode and compare `LVPCFG01`/`LVPMKT01` state and deployed frozen SBF bytes. Value-moving handlers remain hard-locked. |
...
| PROGRAM-05 | Critical | Combining immutable market identity, mutable totals and wallet state in one account would make authority review, long/short isolation and migration unsafe. | Parallel `*02` audit-candidate layouts now split immutable market config, writable accounting, paired risk vault and per-wallet position. Exact encode/decode, checked accounting, reciprocal pair isolation and canonical SBF PDA/role loaders are tested. They remain unused and execution-locked; initialization, migration, transitions and token-authority binding are still open. |
| TOKEN-01 | Critical | One token-program binding was reused for canonical-USDC settlement accounts and LevPlay product accounts, permitting cross-program type confusion when the assets use different token programs. | Config state now pins the USDC program while each market independently pins its product-token program. The exact open layout includes and validates both executable programs; USDC vaults and product mint/accounts can no longer inherit the wrong owner binding. |

...
type Protocol = { executionEnabled: boolean; feeBps: number; maxPilotUsd: number; feeRecipient: string | null; configuredMarkets: string[]; checks: ProtocolCheck[]; blockers: string[] };
type AppView = "trade" | "portfolio" | "history";
type PaperPosition = { id: string; symbol: string; ticker: string; name: string; leverage: number; invested: number; fee: number; net: number; entryPrice: number; openedAt: string };
...
export default function TradingApp() {
  ...
  const [protocol, setProtocol] = useState<Protocol>({ executionEnabled: false, feeBps: 50, maxPilotUsd: 100, feeRecipient: null, configuredMarkets: [], checks: [], blockers: ["Checking release gates"] });
  ...
  const canExecute = protocol.executionEnabled && marketConfigured && oracleReady && amount >= 10 && amount <= protocol.maxPilotUsd;
  const fee = amount * protocol.feeBps / 10_000;
  const net = Math.max(0, amount - fee);
  const exposure = net * leverage;
  const reserve = Math.max(0, exposure - net);
  ...
  const positionValue = useCallback((position: PaperPosition) => {
    ...
    return Math.max(0, position.net * (1 + position.leverage * move));
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        const saved = JSON.parse(window.localStorage.getItem("levplay-paper-v1") || "{}") as { cash?: number; positions?: PaperPosition[]; history?: PaperTrade[] };
      ...
      } catch { window.localStorage.removeItem("levplay-paper-v1"); }
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem("levplay-paper-v1", JSON.stringify({ cash: paperCash, positions, history }));
  ...
  const review = () => {
    ...
    if (!paperMode && balance && balance.usdc < amount) { setNotice(`Insufficient USDC. Available: ${balance.usdc.toFixed(2)} USDC.`); return; }
    if (paperMode && paperCash < amount) { setNotice(`Insufficient paper USDC. Available: ${paperCash.toFixed(2)} USDC.`); return; }
  ...
  const stopPaper = () => { setPaperMode(false); setBalance(null); setNotice("Paper preview closed."); setView("trade"); };
  const openPaperPosition = () => {
    const entryPrice = selected.price || 100;
    const id = paperId();
    const openedAt = new Date().toISOString();
    const position: PaperPosition = { id, symbol: selected.symbol, ticker: selected.ticker, name: selected.name, leverage, invested: amount, fee, net, entryPrice, openedAt };
    setPositions((items) => [position, ...items]);
    setPaperCash(paperCash - amount);
    setBalance({ sol: 0.05, usdc: paperCash - amount });
  ...
  const sellPaperPosition = () => {
    ...
    const pnl = value - exitPosition.invested;
  ...
  return <main className="app-shell">
    ...
    <section className="trading-grid">
      ...
      <aside className="trade-ticket">
        ...
        <label className="amount"><span>Invest</span><div><b>$</b><input value={amount} onChange={(event) => setAmount(Math.min(100, Math.max(0, Number(event.target.value))))} inputMode="decimal" aria-label="Investment amount in US dollars"/><small>USDC</small></div></label>
        <div className="quick-amounts">{[25, 50, 100].map((value) => <button key={value} onClick={() => setAmount(value)}>${value}</button>)}</div>
        ...
        <div className="breakdown"><span><small>Investment</small><strong>{formatUsd(amount)}</strong></span><span><small>LevPlay fee · 0.5%</small><strong>{formatUsd(fee)}</strong></span><span><small>Fee destination</small><strong>{protocol.feeRecipient ? short(protocol.feeRecipient) : "Not configured"}</strong></span><span><small>Net vault equity</small><strong>{formatUsd(net)}</strong></span><span><small>Target exposure</small><strong>{formatUsd(exposure)}</strong></span><span><small>Managed leverage</small><strong>{formatUsd(reserve)}</strong></span></div>
    ...
    <section className="docs-section" id="docs">
      ...
      <div className="docs-cards">
        <article id="how-it-works"><BookOpen/><span><small>01 · Mechanics</small><h3>How it works</h3></span><ol><li>Deposit USDC into one isolated market vault.</li><li>The protocol deducts the disclosed 0.5% entry fee.</li><li>Vault shares mint from conservative net asset value.</li><li>Permissionless keepers restore the daily leverage target inside strict bounds.</li><li>Redeeming burns shares before value leaves the vault.</li></ol></article>
      ...
      <div className="faq-grid">
        ...
        <details id="treasury"><summary>Where does the 0.5% fee go?</summary><p>In this preview it is calculated for testing but no funds move. Mainnet must send USDC directly to the configured multisig treasury account; the destination will be displayed before signing and verifiable on Solana.</p></details>
    ...
    <Dialog open={reviewOpen} onOpenChange={setReviewOpen}><DialogContent className="review-dialog"><DialogHeader><DialogTitle>Review {productId}</DialogTitle><DialogDescription>{paperMode ? "Paper preview only. No funds or transactions will move." : "A capped, self-custodial leveraged-token position on Solana."}</DialogDescription></DialogHeader><div className="review-asset"><span style={{ background: selected.tone }}>{selected.ticker[0]}</span><div><strong>{selected.name} {leverage}× Long</strong><small>{selected.symbol} · xStocks Token-2022</small></div><b>{formatUsd(amount)}</b></div><div className="review-grid"><span><small>Fee</small><strong>{formatUsd(fee)}</strong></span><span><small>Target exposure</small><strong>{formatUsd(exposure)}</strong></span><span><small>Oracle guard</small><strong>{selected.verified ? "Pyth + Chainlink" : "Unavailable"}</strong></span><span><small>Pilot limit</small><strong>$100 / wallet</strong></span></div>{paperMode ? <div className="gate-box paper"><PlayCircle/><div><strong>Paper preview</strong><p>This records a local practice position and never submits a Solana transaction.</p></div></div> : <><div className={canExecute ? "gate-box ready" : "gate-box"}>{canExecute ? <ShieldCheck/> : <LockKeyhole/>}<div><strong>{canExecute ? "All release gates passed" : "Transaction signing is disabled"}</strong><p>{canExecute ? "Your wallet will show the exact Solana instructions before signing." : (protocol.blockers[0] || (!oracleReady ? "Dual-oracle verification failed." : `${productId} is not an audited deployment.`))}</p></div></div>{!canExecute && <div className="gate-list">{protocol.blockers.slice(0, 5).map((blocker) => <span key={blocker}><i/>{blocker}</span>)}</div>}<Progress value={canExecute ? 100 : Math.max(12, 100 - protocol.blockers.length * 13)} /></>}<button className="review-button" onClick={paperMode ? openPaperPosition : undefined} disabled={!paperMode && !canExecute}>{paperMode ? "Open paper position" : canExecute ? "Sign Solana transaction" : "Mainnet safety lock active"}</button><p className="dialog-note">Liquidation-free describes the holder experience, not risk-free returns. Rebalancing, oracle, liquidity, custody and smart-contract risks remain.</p></DialogContent></Dialog>
    <Dialog open={Boolean(exitPosition)} onOpenChange={(open) => { if (!open) setExitPosition(null); }}><DialogContent className="review-dialog"><DialogHeader><DialogTitle>Sell {exitPosition ? `${exitPosition.ticker}${exitPosition.leverage}L` : "position"}</DialogTitle><DialogDescription>Close this paper position and record the realized result.</DialogDescription></DialogHeader>{exitPosition && <><div className="review-grid"><span><small>Invested</small><strong>{formatUsd(exitPosition.invested)}</strong></span><span><small>Estimated proceeds</small><strong>{formatUsd(positionValue(exitPosition))}</strong></span><span><small>Entry reference</small><strong>{formatUsd(exitPosition.entryPrice)}</strong></span><span><small>Realized P/L</small><strong className={positionValue(exitPosition) - exitPosition.invested >= 0 ? "positive" : "negative"}>{positionValue(exitPosition) - exitPosition.invested >= 0 ? "+" : ""}{formatUsd(positionValue(exitPosition) - exitPosition.invested)}</strong></span></div><button className="review-button" onClick={sellPaperPosition}>Confirm paper sale</button></>}</DialogContent></Dialog>
  </main>;
}

...
export default function TradingApp() {
  ...
  useEffect(() => {
    ...
    void register({ name: "stage_levplay_position", title: "Stage position", description: "Configure, but never submit, a capped leveraged-token order for review.", inputSchema: { type: "object", properties: { amountUsd: { type: "number", minimum: 10, maximum: 100 }, leverage: { type: "integer", enum: [2, 3] }, direction: { type: "string", enum: ["Long", "Short"] } }, required: ["amountUsd", "leverage", "direction"], additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute(input) { const value = input as { amountUsd?: number; leverage?: number; direction?: Direction }; if (!value.amountUsd || value.amountUsd < 10 || value.amountUsd > 100 || ![2, 3].includes(value.leverage || 0) || !["Long", "Short"].includes(value.direction || "")) throw new Error("Invalid pilot order"); setAmount(value.amountUsd); setLeverage(value.leverage!); setDirection(value.direction!); setReviewOpen(true); return { staged: true, amountUsd: value.amountUsd, leverage: value.leverage, direction: value.direction, submitted: false }; } });
  ...
  const categoryCounts = useMemo(() => ({ Stocks: markets.filter((market) => market.category === "Stocks").length, "Hong Kong": markets.filter((market) => market.category === "Hong Kong").length, "Pre-IPO": markets.filter((market) => market.category === "Pre-IPO").length, Commodities: markets.filter((market) => market.category === "Commodities").length }), [markets]);
  ...
  return <main className="app-shell">
    ...
    {view === "trade" && <><section className="market-hero">
      <div><span className="eyebrow">US + Hong Kong stocks via xStocks · pre-IPO references via PreStocks · Solana</span><h1>Trade leveraged tokens.<br/><em>Not margin accounts.</em></h1></div>
    ...
    <section className="trading-grid">
      ...
      <div className="detail-panel">
        <div className="detail-top"><div className="selected-asset"><AssetLogo market={selected} size={58}/><div><small>LevPlay · {selected.category}</small><h2>{selected.ticker}<em>{leverage}{direction === "Long" ? "L" : "S"}</em></h2><p>Liquidation-free holder structure</p></div></div><div className="selected-price"><small>{selected.category === "Pre-IPO" ? "PreStocks market reference" : "Underlying xStock reference"}</small><strong>{selected.price ? formatUsd(selected.price) : selected.category === "Hong Kong" && selected.period === "closed" ? "Market closed" : "Reference offline"}</strong><span>{selected.category === "Pre-IPO" ? "Live DEX display · execution locked" : selected.marketOpen ? "Primary session open" : selected.category === "Hong Kong" && selected.period === "closed" ? "HKEX session closed" : "24/7 secondary market"}</span></div></div>
      ...
      <aside className="trade-ticket">
        ...
        <div className="leverage-buttons">{[2, 3].map((value) => <button key={value} className={leverage === value ? "active" : ""} onClick={() => setLeverage(value)}>{value}×</button>)}</div>
    ...
    <footer className="site-footer">
      ...
      <nav><strong>Launch status</strong><span>{protocol.executionEnabled ? "Mainnet gates passed" : "Preview · signing locked"}</span><span>{verifiedCount}/{markets.length} references pass live data checks</span><span>2× and 3× · long and short · individually gated</span></nav>

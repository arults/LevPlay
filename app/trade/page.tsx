  ...
  return <main className="app-shell">
    ...
    <section className="trading-grid">
      ...
      <div className="detail-panel">
        <div className="detail-top"><div className="selected-asset"><span style={{ background: selected.tone }}>{selected.ticker[0]}</span><div><small>LevPlay · {selected.category}</small><h2>{selected.ticker}<em>{leverage}{direction === "Long" ? "L" : "S"}</em></h2><p>Liquidation-free leveraged token</p></div></div><div className="selected-price"><small>Underlying xStock reference</small><strong>{selected.price ? formatUsd(selected.price) : "—"}</strong><span>{selected.marketOpen ? "Primary session open" : "24/7 secondary market"}</span></div></div>
    ...
    <footer className="site-footer">
      ...
      <nav><strong>Launch status</strong><span>{protocol.executionEnabled ? "Mainnet gates passed" : "Preview · signing locked"}</span><span>{verifiedCount}/15 references pass live data checks</span><span>AAPL2L + AAPL2S audit pilot · $100 per wallet</span></nav>

...
import { Activity, ArrowUpRight, BookOpen, BriefcaseBusiness, Check, ChevronRight, CircleAlert, Code2, ExternalLink, Eye, History as HistoryIcon, Info, Layers3, LoaderCircle, LockKeyhole, LogOut, PlayCircle, RefreshCw, RotateCw, Scale, Search, ShieldCheck, Sparkles, TrendingDown, TrendingUp, Wallet, Zap } from "lucide-react";
...
export default function TradingApp() {
  ...
  useEffect(() => {
    ...
    void register({ name: "select_levplay_market", title: "Select market", description: "Select one supported pre-IPO reference in the visible LevPlay trade ticket.", inputSchema: { type: "object", properties: { symbol: { type: "string", enum: ALL_MARKETS.map((market) => market.symbol) } }, required: ["symbol"], additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute(input) { const symbol = String((input as { symbol?: string }).symbol || ""); if (!ALL_MARKETS.some((market) => market.symbol === symbol)) throw new Error("Unsupported market"); setSelectedSymbol(symbol); return { selected: symbol }; } });
    void register({ name: "stage_levplay_position", title: "Stage position", description: "Configure, but never submit, a capped 2x leveraged-token order for review.", inputSchema: { type: "object", properties: { amountUsd: { type: "number", minimum: 10, maximum: 100 }, leverage: { type: "integer", enum: [2] }, direction: { type: "string", enum: ["Long", "Short"] } }, required: ["amountUsd", "leverage", "direction"], additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute(input) { const value = input as { amountUsd?: number; leverage?: number; direction?: Direction }; if (!value.amountUsd || value.amountUsd < 10 || value.amountUsd > 100 || value.leverage !== 2 || !["Long", "Short"].includes(value.direction || "")) throw new Error("Invalid pilot order"); setAmount(value.amountUsd); setLeverage(2); setDirection(value.direction!); setReviewOpen(true); return { staged: true, amountUsd: value.amountUsd, leverage: 2, direction: value.direction, submitted: false }; } });
    return () => controller.abort();
  }, []);

  const visible = useMemo(() => markets.filter((market) => market.provider === preIpoProvider && `${market.ticker} ${market.name}`.toLowerCase().includes(query.toLowerCase())), [markets, preIpoProvider, query]);
  ...
  return <main className="app-shell">
    ...
    <section className="trading-grid">
      ...
      <div className="detail-panel">
        <div className="detail-top"><div className="selected-asset"><AssetLogo market={selected} size={58}/><div><small>LevPlay · Pre-IPO</small><h2>{selected.ticker}<em>{leverage}{direction === "Long" ? "L" : "S"}</em></h2><p>Liquidation-free holder structure</p></div></div><div className="selected-price"><small>{selected.provider === "PreStocks" ? "PreStocks Token Price" : `${selected.provider} issuer mark`}</small><strong>{selected.price ? formatUsd(selected.price) : selected.referenceLabel || "Reference unavailable"}</strong><span>{selected.dexPrice && selected.provider === "PreStocks" ? `DEX pool ${formatUsd(selected.dexPrice)} · separate market` : selected.referenceLabel || "Checking reference provider"} · settlement {selected.verified ? "ready" : "locked"}</span></div></div>
      ...
      <aside className="trade-ticket">
        ...
        <div className="leverage-buttons">{[2].map((value) => <button key={value} className={leverage === value ? "active" : ""} onClick={() => setLeverage(value)}>{value}×</button>)}</div>
    ...
    <section className="docs-section" id="docs">
      ...
      <div className="faq-grid">
        ...
        <details><summary>Who controls the source token?</summary><p>PreStocks and Tessera may retain issuer controls. Tessera T-Tokens are unsecured loan participation rights rather than equity; PreStocks products provide economic exposure without shareholder rights. LevPlay treats both providers as external trust boundaries, stops new mints on source changes, isolates every product and preserves close-only pro-rata exits.</p></details>
    ...
    <footer className="site-footer">
      ...
      <nav><strong>Resources</strong><a href="https://prestocks.com/products" target="_blank" rel="noreferrer">PreStocks products <ExternalLink size={11}/></a><a href="https://docs.tessera.pe/overview/how-do-tessera-token-work" target="_blank" rel="noreferrer">Tessera docs <ExternalLink size={11}/></a><a href="https://solana.com/docs" target="_blank" rel="noreferrer">Solana docs <ExternalLink size={11}/></a></nav>
      <nav><strong>Launch status</strong><span>{protocol.executionEnabled ? "Mainnet gates passed" : "Preview · signing locked"}</span><span>{verifiedCount}/{markets.length} settlement-ready · {displayCount}/{markets.length} display references online</span><span>2× long and short · individually gated</span></nav>
      <div className="footer-legal"><span>© 2026 LevPlay</span><p>PreStocks and Tessera products are economic-exposure instruments subject to issuer controls, liquidity risks and jurisdiction restrictions. They do not confer direct ownership of the referenced company. Availability does not imply eligibility.</p></div>
    </footer>

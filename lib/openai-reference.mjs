...
export function quoteOpen({ provider, mint, side, capital, observation: mark, capacity, expirySlot }) {
  source(provider, mint);
  if (side !== "long" && side !== "short") throw new Error("invalid side");
  amount(capital, "capital", true);
  amount(capacity, "maker capacity");
  ...
  if (makerRequired > capacity) throw new Error("maker collateral insufficient");
...
export function openPosition(input) {
  const { owner, nonce, observation: mark, capital, provider, mint, side, capacity, expirySlot } = input;
  if (typeof owner !== "string" || !owner.trim() || typeof nonce !== "string" || !nonce.trim())
    throw new Error("invalid position identity");
  const quote = quoteOpen({ provider, mint, side, capital, observation: mark, capacity, expirySlot });

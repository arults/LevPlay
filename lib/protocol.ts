...
async function releaseInputs(nowUnix: number) {
  ...
  try {
    ...
    if (!venue.admitted || admissions.some((item) => !item.admitted) || ids.join(",") !== "ANTH2L,ANTH2S") throw new Error("venue or exact pilot products are not admitted");
...
export async function protocolStatus(nowUnix = Math.floor(Date.now() / 1_000)) {
  ...
  const checks = [
    ...
    { id: "products", label: "Exact ANTH2L/ANTH2S manifests", passed: Boolean(manifest && release.productIds.join(",") === "ANTH2L,ANTH2S"), failure: "Exact pilot product manifests are not admitted and release-bound" },

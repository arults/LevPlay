...
export type ReleaseManifestV2 = {
  ...
  markets: { AAPL2L: ReleaseMarket; AAPL2S: ReleaseMarket };
};
...
function market(value: unknown, productId: "AAPL2L" | "AAPL2S"): ReleaseMarket {
  ...
  if (item.sourceProvider !== "ondo" || item.sourceAssetSymbol !== "AAPLon" || item.leverage !== 2 ||
      item.side !== (productId === "AAPL2L" ? "long" : "short") ||
...
export function parseReleaseManifestV2(source: string): ReleaseManifestV2 {
  ...
  exactKeys(markets, ["AAPL2L", "AAPL2S"], "markets");
  const parsed = {
    ...root,
    releaseArtifacts: artifacts,
    evidence,
    markets: { AAPL2L: market(markets.AAPL2L, "AAPL2L"), AAPL2S: market(markets.AAPL2S, "AAPL2S") },
  } as ReleaseManifestV2;
  if (parsed.markets.AAPL2L.marketState === parsed.markets.AAPL2S.marketState ||
      parsed.markets.AAPL2L.productMint === parsed.markets.AAPL2S.productMint ||
      parsed.markets.AAPL2L.clearingVault === parsed.markets.AAPL2S.clearingVault ||
      parsed.markets.AAPL2L.sourceVault === parsed.markets.AAPL2S.sourceVault ||
      parsed.markets.AAPL2L.reserveVault === parsed.markets.AAPL2S.reserveVault ||
      parsed.markets.AAPL2L.adapterMarket === parsed.markets.AAPL2S.adapterMarket) {

...
assert.ok(schema.$defs.baseMarket.required.includes("clearingVault"));
assert.ok(schema.$defs.baseMarket.required.includes("sourceVault"));
...
assert.equal(schema.properties.upgradePolicy.const, "frozen");
assert.ok(schema.required.includes("evidence"));
assert.ok(schema.required.includes("configState"));

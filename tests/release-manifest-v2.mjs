...
import { ONDO_SOLANA, MINIMUM_BLOCKED_COUNTRIES } from "../lib/venue-registry.ts";
...
const market = (side, seed) => ({
  ...
  sourceProvider: "ondo", sourceAssetSymbol: "AAPLon", sourceMint: ONDO_SOLANA.aaplMint, sourceMintAccountSha256: hash(sourceMintBytes), sourceRegistryHash: h,
...
const product = (productId, item) => ({
  productId, provider: "ondo", sourceSymbol: "AAPLon", sourceMint: item.sourceMint, productMint: item.productMint, marketPda: item.marketState,
...
const productsSource = JSON.stringify([product("AAPL2L", long), product("AAPL2S", short)]);
const venueSource = JSON.stringify({
  venueId: ONDO_SOLANA.id, chainId: ONDO_SOLANA.chainId, apiOrigin: ONDO_SOLANA.apiOrigin, gmProgramId: ONDO_SOLANA.gmProgramId,
  orderEngineProgramId: ONDO_SOLANA.jupiterOrderEngineProgramId, assetSymbol: "AAPLon", assetMint: ONDO_SOLANA.aaplMint, collateralMint: ONDO_SOLANA.usdcMint,
  tokenProgramId: ONDO_SOLANA.tokenProgramId, authorizedSolvers: [...ONDO_SOLANA.authorizedSolvers], allowedCountries: ["IN"], blockedCountries: [...MINIMUM_BLOCKED_COUNTRIES],
  eligibilityAttestor: address(81), eligibilityPolicyHash: h, legalApprovalHash: h, wrapperApprovalHash: h, independentAuditHash: h, auditorRetestHash: h,
...
const release = {
  ...
  feeRecipient, treasuryAuthority: treasury, governance, guardian, multisigProgram, markets: { AAPL2L: long, AAPL2S: short },
};
const releaseSource = JSON.stringify(release);
assert.equal(parseReleaseManifestV2(releaseSource).markets.AAPL2S.side, "short");
assert.throws(() => parseReleaseManifestV2(JSON.stringify({ ...release, surprise: true })), /missing or extra/);
assert.throws(() => parseReleaseManifestV2(JSON.stringify({ ...release, markets: { AAPL2L: long } })), /missing or extra/);
assert.throws(() => parseReleaseManifestV2(JSON.stringify({ ...release, markets: { AAPL2L: long, AAPL2S: { ...short, sourceVault: short.clearingVault } } })), /aliased/);
...
try {
  ...
  const extra = JSON.stringify([...JSON.parse(productsSource), product("AAPL2L", long)]); process.env.LEVPLAY_SVM_PRODUCT_MANIFESTS_JSON = extra;

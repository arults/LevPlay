...
assert.match(chart, /Neither can settle a LevPlay position/);
assert.match(marketApi, /const price = Number\(issuer\?\.tokenPrice\)/);
assert.doesNotMatch(marketApi, /const price = Number\(dex\?\.price\)/);
